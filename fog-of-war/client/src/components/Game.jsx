...
export default function Game() {
  ...
  useEffect(() => {
    const computeScale = () => {
      const available = Math.min(window.innerWidth, window.innerHeight - 52);
      const scale = Math.min(1, available / 2048);
      const mobile = window.innerWidth < 768;
      console.log('computeScale:', { width: window.innerWidth, scale, mobile });
  ...
  // ── Move player function (used by keyboard and mobile controls) ──────────
  useEffect(() => {
    movePlayerRef.current = (dx, dy) => {
      console.log('movePlayer called:', { dx, dy });
      const { localMode, myPos } = stateRef.current;
      if (!myPos) return;
    ...
    console.log('movePlayerRef.current initialized');
  }, []);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      Object.entries(players || {}).forEach(([id, p], idx) => {
        ...
        const drawn = drawPlayerSprite(ctx, playerSpriteRef.current, spriteX, spriteY, spriteSize, charIndex);
      ...
      console.log('Drawing player sprite:', { playerSprite: playerSpriteRef.current, myCharIndex, mySpriteX, mySpriteY });
      const myDrawn = drawPlayerSprite(ctx, playerSpriteRef.current, mySpriteX, mySpriteY, mySpriteSize, myCharIndex);
  ...
  console.log('Game render:', { isMobile, canvasScale });
  ...
  return (
    <div className="game">
      ...
      {isMobile && (
        <>
          {console.log('Rendering MobileControls, movePlayerRef.current:', movePlayerRef.current)}
          <MobileControls
            onMove={movePlayerRef.current ? (dx, dy) => movePlayerRef.current(dx, dy) : () => {}}
          onAttack={() => {
            if (stateRef.current.localMode) {
              // local mode attack vs bots
              const { myPos } = stateRef.current;
              if (!myPos) return;
              // Find nearest bot within attack range
              const nearestBot = botsRef.current.find(bot => {
                if (bot.status !== 'alive') return false;
                const d = Math.abs(bot.x - myPos.x) + Math.abs(bot.y - myPos.y);
                return d <= 1;
              });
              if (nearestBot) {
                flashRef.current = Date.now();
                // Damage is handled by bot AI tick
              }
            } else {
              // multiplayer attack vs players
              const { players, myId, myPos } = stateRef.current;
              if (!myPos || !players) return;
              let nearest = null;
              let nearestDist = Infinity;
